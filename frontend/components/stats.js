...
function Card({ children, className = "" }) {
  return <div className={`rounded-xl border shadow-lg backdrop-blur-sm ${className}`}>{children}</div>
}
...
export default function StatsDashboard({ stats }) {
  ...
  const chartData = Object.entries(safeStats.project_chart).map(([timestamp, count]) => {
    try {
  ...
  const growthPercentage = firstValue > 0 ? ((growth / firstValue) * 100).toFixed(1) : "0.0"
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <Card className="relative overflow-hidden bg-gradient-to-br from-gray-900/80 to-gray-800/80 border-gray-700/50 hover:border-blue-500/50 transition-all duration-300 group">
          ...
          <CardContent className="px-4 pb-4">
            <div className="text-2xl font-bold text-blue-400 mb-1">{safeStats.total_projects.toLocaleString()}</div>
        ...
        <Card className="relative overflow-hidden bg-gradient-to-br from-gray-900/80 to-gray-800/80 border-gray-700/50 hover:border-green-500/50 transition-all duration-300 group">
          ...
          <CardContent className="px-4 pb-4">
            <div className="text-2xl font-bold text-green-400 mb-1">{safeStats.total_users.toLocaleString()}</div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 10 }}>
                ...
                <Tooltip
                  ...
                  formatter={(value) => [`${value} projects`, "Total Projects"]}
                ...
                <div className="flex flex-col items-center text-center space-y-2">
                  ...
                  <div className="space-y-1 w-full">
                    ...
                    <div className="flex items-center justify-center gap-1">
                      ...
                      <span className="text-xs text-gray-500">{user.projects || 0} projects</span>
                    </div>
                  </div>
                      ...
                      style={{
                        width: `${
                          safeStats.top10_users[0]?.projects > 0
                            ? ((user.projects || 0) / safeStats.top10_users[0].projects) * 100
                            : 0
                        }%`,
                      }}
                    ></div>
